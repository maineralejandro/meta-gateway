...
async def test_e2e_inference_generates_spanish_response(e2e_client):
    ...
    response = await client.chat.completions.create(
        ...
        max_tokens=200,
        messages=[
            {"role": "system", "content": "Eres un asistente de un food truck chileno. Responde en español."},
            {"role": "user", "content": "<customer_message>\nHola, quiero un completo italiano\n</customer_message>"},
        ],
    )
    text = response.choices[0].message.content.strip()
    assert len(text) > 10
    assert any(w in text.lower() for w in ["completo", "italiano", "pedido", "anotado", "hola"])


@pytest.mark.asyncio
@skip_unless_e2e
async def test_e2e_inference_uses_order_tags(e2e_client):
    client, model = e2e_client
    response = await client.chat.completions.create(
        model=model,
        max_tokens=300,
        messages=[
            {
                "role": "system",
                "content": (
                    "Eres Hermes, bot de un food truck chileno. "
                    "Responde en español. Usa tags [ORDER_ADD:item_key:qty] para agregar al pedido. "
            ...
            {"role": "user", "content": "<customer_message>\nQuiero 2 completos italianos y una papas mediana\n</customer_message>"},
        ],
    )
    text = response.choices[0].message.content.strip()
    assert "[ORDER_ADD:" in text or "ORDER_ADD" in text
